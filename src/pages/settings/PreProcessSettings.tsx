...
export default function PreProcessSettings() {
  ...
  if (!config) {
    return <div className="text-text-secondary p-6">{t('settings.preProcess.loading')}</div>;
  }

  return (
    <div className="space-y-5">
      <div>
        <h1 className="ui-page-title">{t('settings.preProcess.pageTitle')}</h1>
        <p className="ui-page-desc">{t('settings.preProcess.pageDesc')}</p>
      </div>

      {/* Global Options */}
      <Card
        title={t('settings.preProcess.globalTitle')}
        description={t('settings.preProcess.globalDesc')}
      >
        <div className="space-y-3">
          <ToggleRow
            label={t('settings.preProcess.trim')}
            description={t('settings.preProcess.trimDesc')}
            checked={config.trimWhitespace}
            onChange={() => void handleToggleGlobal('trimWhitespace')}
          />
          <ToggleRow
            label={t('settings.preProcess.unicode')}
            description={t('settings.preProcess.unicodeDesc')}
            checked={config.normalizeUnicode}
            onChange={() => void handleToggleGlobal('normalizeUnicode')}
          />
          <ToggleRow
            label={t('settings.preProcess.control')}
            description={t('settings.preProcess.controlDesc')}
      ...
      {/* Rules */}
      <Card
        title={t('settings.preProcess.rulesTitle', { count: (config.rules ?? []).length })}
        description={t('settings.preProcess.rulesDesc')}
      >
        <div className="space-y-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {rule.langPair && rule.langPair !== 'all' && (
                  <span className="text-xs text-text-tertiary mt-1 inline-block">
                    {t('settings.preProcess.onlyLang', { lang: rule.langPair })}
                  </span>
                )}
              </div>
          ...
          {showAddForm ? (
            <div className="p-3 border border-primary/30 rounded-lg bg-bg-primary space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-medium text-text-secondary mb-1">
                    {t('settings.preProcess.replaceWith')}
                  </label>
                  <input
                    type="text"
              ...
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-2 text-sm">
                  <input
                    ...
                    className="rounded"
                  />
                  <span className="text-text-secondary">{t('settings.preProcess.regex')}</span>
                </label>
                <div className="flex-1">
                  <input
                    ...
                    placeholder={t('settings.preProcess.langFilter')}
            ...
            >
              <Plus size={16} />
              <span className="text-sm">{t('settings.preProcess.addRule')}</span>
            </button>
          )}
        </div>
      </Card>
      ...
      {/* Test */}
      <Card title={t('settings.preProcess.testTitle')} description={t('settings.preProcess.testDesc')}>
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-text-secondary mb-1">
                {t('settings.preProcess.input')}
              </label>
              <textarea
                value={testInput}
