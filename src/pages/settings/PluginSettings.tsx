...
function Plugins() {
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      case 'translation':
        return t('plugins.typeTranslation');
      case 'ocr':
        return t('plugins.typeOcr');
      case 'tts':
        return t('plugins.typeTts');
      default:
        return type;
    }
  };
...
{ "translated": "你好" }`}
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full ${getTypeColor(plugin.manifest.type)}`}
                      >
        ...
        {plugins.length > 0 && (
          <p className="text-xs text-text-secondary text-center mt-4">{t('plugins.restartHint')}</p>
        )}
      </div>
    </div>
  );
