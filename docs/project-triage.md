...
| P1 | Browser extension | Chrome and Firefox code paths diverged; old `firefox-extension/` still exists | Decide one-source build vs two maintained trees, then remove or archive the unused path | One documented install path per browser; no stale manifest/icon references | **Fixed** - Removed `firefox-extension/`; `extension/` now unified for both browsers via `build.js` |
