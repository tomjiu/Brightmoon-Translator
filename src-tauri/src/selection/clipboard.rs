...
impl SelectionProvider for ClipboardSelectionProvider {
    async fn get_selection(&self) -> Option<SelectionResult> {
        log::debug!("[clipboard] Attempting clipboard selection via Ctrl+C");
        let (text, window_title) = match get_clipboard_selection() {
            Some(r) => r,
            None => {
                log::debug!("[clipboard] get_clipboard_selection returned None");
                return None;
            }
        };
        if text.trim().is_empty() {
            log::debug!("[clipboard] Got text but it's empty/whitespace");
            return None;
        }
        log::info!("[clipboard] Success: {} chars from '{}'", text.trim().len(), window_title);
...
fn get_clipboard_selection() -> Option<(String, String)> {
    #[cfg(target_os = "windows")]
    {
        ...
        unsafe {
            ...
            if !clipboard_ready {
                log::debug!("[clipboard] Adaptive wait: no content after 500ms, extra 100ms fallback");
                std::thread::sleep(std::time::Duration::from_millis(100));
            } else {
                log::debug!("[clipboard] Adaptive wait: clipboard has content");
            }
            ...
            let selected_text = if OpenClipboard(std::ptr::null_mut()) != 0 {
                ...
                log::debug!("[clipboard] Failed to open clipboard for reading");
                None
            };

            match &selected_text {
                Some(t) => log::debug!("[clipboard] Read {} chars from clipboard", t.len()),
                None => log::debug!("[clipboard] No text in clipboard after Ctrl+C"),
            }
