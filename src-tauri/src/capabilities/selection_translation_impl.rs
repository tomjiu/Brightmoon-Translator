...
///
/// Strategy: full provider chain (UIA → clipboard) for all apps.
/// Modern Electron/WebView2/CEF apps often support UIA TextPattern.
...
impl SelectionTranslation for DefaultSelectionTranslation {
    async fn translate_selection(
        ...
        // Step 1: Detect foreground app for overlay context
        let app_ctx = self.app_detector.detect().await;

        // Step 2: Get selection — full provider chain for all apps.
        // Modern Electron/WebView2/CEF apps often support UIA TextPattern,
        // so we try UIA first and let clipboard act as fallback naturally.
        ...
        log::info!("[selection_translate] Completed via provider '{}'", provider);
